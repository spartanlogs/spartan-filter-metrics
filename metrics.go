...
import (
	...
	"github.com/spartanlogs/spartan/metrics"
	"github.com/spartanlogs/spartan/utils"
)

func init() {
	filters.Register("metrics", newMetricsFilter)
}
...
func newMetricsFilter(options utils.InterfaceMap) (filters.Filter, error) {
...
func (f *MetricsFilter) Filter(batch []*event.Event, matchedFunc filters.MatchFunc) []*event.Event {
	for range batch {
		f.meter.Mark(1)
	}
	return batch
